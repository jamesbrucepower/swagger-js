...
var Operation = module.exports = function (parent, scheme, operationId, httpMethod, path, args, definitions, models, clientAuthorizations) {
  ...
  for (i = 0; i < this.parameters.length; i++) {
    ...
    param.sampleRequest = this.getModelRequest();
...
Operation.prototype.getModelRequest = function() {
  var sampleRequest = '';
  var spanOpen = '<span class="propType">';
  var spanClose = '</span>';
  var space = '&nbsp;';

  sampleRequest += spanOpen + this.method.toUpperCase() + spanClose;
  sampleRequest += space + this.method.path + space;
  sampleRequest += 'HTTP/1.1\n';
  sampleRequest += JSON.stringify(this.getRealHeaderParams(this.getHeaderParams()));
